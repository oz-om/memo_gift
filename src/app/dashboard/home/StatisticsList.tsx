...
export default async function StatisticsList() {
  ...
  return (
    <div className='orders_today_wrapper grid grid-cols-1 min-[300px]:grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 place-content-center '>
      ...
      <StatisticOrder name='complete' icon='bx-line-chart' value={`${(allShippedOrders.length * 100) / allOrders.length}%`} className='orders_complete_percentage from-violet-600 to-violet-100 bg-gradient-to-br' />
    </div>
  );
}
